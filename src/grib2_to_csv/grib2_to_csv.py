...
def get_MSM_index_lat_lon():
    ...
    MSM_mesh_index = lat_mesh.str.cat(lon_mesh, sep='-')  # 緯度と経度の座標番号を'-'で組み合わせ
...
def get_MSM_list(df_meshcode):
    ...
    x1 = (df_meshcode['meshcode'].astype(str).str[2] + df_meshcode['meshcode'].astype(str).str[3]).astype(int).values
    ...
    lat = ((y1 * 80 + y2 * 10 + y3) * 30 / 3600) + 15 / 3600  # 南西方向の座標からメッシュ中心の緯度を算出
    lon = (((x1 * 80 + x2 * 10 + x3) * 45 / 3600) + 100) + 22.5 / 3600  # 南西方向の座標からメッシュ中心の経度を算出

    ### メッシュ周囲のMSM位置（緯度経度）と番号（北始まり0～、西始まり0～）の取得
    lat_N = np.ceil(lat / lat_unit) * lat_unit  # 北は切り上げ
    ...
    lon_E = np.ceil(lon / lon_unit) * lon_unit  # 東は切り上げ
    MSM_E = np.round((lon_E - 120) / lon_unit).astype(int)

    df = pd.DataFrame()  # 保存用のDFを作成
    ...
    df['index'] = df['MSM_no_UD'].astype(str) + '-' + df['MSM_no_LR'].astype(str)
...
def func_MR(t, RH, P):
    ...
    eSAT = np.exp( -5800.2206 / T + 1.3914993 - 0.048640239 * T + 0.41764768 * 10 ** (-4) * T**2\
               -0.14452093 * 10 ** (-7) * T**3 + 6.5459673 * np.log( T )) / 100
    aT = ( 217 * eSAT) / T
    VH = aT * ( RH / 100)
...
def func_IHO_AM(day_date, phi, lon):
    ...
    for j in count:
        ...
        trad = np.radians(t)
        sinh = pd.DataFrame(np.sin(phirad) * sindlt + np.cos(phirad) * cosdlt * np.cos(trad),
        ...
        if (sinh['sinh'] >= 0).sum() == 0:
            ...
            sinh.loc[sinh['sinh'] >= 0.0, 'AM'] = 1 / (sinh['sinh'] + 0.15 * \
...
def oder_select_rad(df, oder):
    return df.at[oder, 'a'], df.at[oder, 'b'], df.at[oder, 'c'], df.at[oder, 'd'], df.at[oder, 'e'], df.at[oder, 'f'], \
           df.at[oder, 'g']

@jit(nopython=True)
def func_TH(clu_data, clm_data, cll_data, AM, TT, RH, a, b, c, d, e, f, g, IHO):
    return (a * clu_data + b * clm_data + c * cll_data + d * AM + e * TT + f * RH + g) * IHO

def func_radiation(date, lat, lon, HCDC, MCDC, LCDC, TCDC, TMP, RH, df_coef_rad):
    ### 大気外日射量とエアマスの計算
    IHO, AM = func_IHO_AM(date, lat, lon)

    ### 計算用のdfを作成
    df_rad = pd.DataFrame(list(zip(HCDC, MCDC, LCDC, TCDC, AM, TMP, RH, IHO)),
    ...
    for name, group in df_group:
        ...
        df_rad.loc[g_index, 'DSWRF_est'] = func_TH(df_rad.loc[g_index, 'HCDC'].values,
...
def oder_select_atm(df,oder):
    return df.at[oder, 'u'],df.at[oder, 'v'],df.at[oder, 'w'],df.at[oder, 'x']
...
def func_e(TMP,RH):
    TMP_abs = TMP+ 273.15
    eSAT = np.exp( -5800.2206 / TMP_abs + 1.3914993 - 0.048640239 * TMP_abs + 0.41764768 * 10 ** (-4) * TMP_abs**2\
               -0.14452093 * 10 ** (-7) * TMP_abs**3 + 6.5459673 * np.log( TMP_abs )) / 100
    return eSAT * RH / 100

@jit("f8[:](f8[:],f8[:],f8[:])",nopython=True)
def func_Ld(TMP,e,C):
    TMP_abs = TMP+ 273.15
    SG = 5.67*10**(-8) #ステファンボルツマン定数

    Tdew = 237.3 * np.log10( e / 6.11 ) / ( 7.5 - np.log10( e / 6.11 )) #露点温度
    x = 0.0315 * Tdew - 0.1836
    Ldf = ( 0.74 + 0.19 * x + 0.07 * x ** 2 ) * SG * TMP_abs ** 4
    return SG * TMP_abs ** 4 * ( 1 - ( 1 - Ldf / ( SG * TMP_abs ** 4))* C)

def func_a_radiation(TMP,RH,LCDC5,LCDC25,HMCDC5,HMCDC25,APCP01,df_coef_atm):

    ### 大気放射量の補間計算
    e = func_e(TMP,RH)

    ### 計算用のdfを作成
    df_rad = pd.DataFrame(list(zip(LCDC5,LCDC25,HMCDC5,HMCDC25,e,APCP01)),
...
def calc_VF(l, b, d):
    return 1 / (2 * pi) * (
                l / (sqrt(d ** 2 + l ** 2)) * atan(b / sqrt(d ** 2 + l ** 2)) + b / (sqrt(d ** 2 + b ** 2)) * atan(
            l / (sqrt(d ** 2 + b ** 2))))
...
def viewfactor(H):
    ...
    for num in range(len(N)):
        ...
        if N[num] == 2:
            VF[num] = calc_VF(y1[num], x1[num], H) * 2 - calc_VF(y1[num], x2[num], H) * 2

        if N[num] == 3:
            VF[num] = calc_VF(y1[num], x1[num], H) * 2 - calc_VF(y2[num], x1[num], H) * 2

        if N[num] == 4:
            VF[num] = calc_VF(y1[num], x1[num], H) - calc_VF(y2[num], x1[num], H) \
                      - calc_VF(y1[num], x2[num], H) + calc_VF(y2[num], x2[num], H)
...
save_list = ['TMP', 'MR', 'DSWRF_est', 'DSWRF_msm', 'Ld', 'VGRD', 'UGRD', 'PRES', 'APCP01']
save_name = ['TMP', 'MR', 'DSWRF_est', 'DSWRF_msm', 'Ld', 'VGRD', 'UGRD', 'PRES', 'APCP01']

# 補間計算ループ
def hokanloop(df, date_end, df_coef_atm, df_coef_rad, file_list, temp_path, start_date, logger):
    """
    ...
    for loop in range(len(file_list)):
        ...
        # 計算対象日時 の計算
        if date_2 is np.nan:  # 1回目は回避
            date_2 = date_end
            date_list.append(date_2)
            
            if date_2 <= start_date:
                break
            
        else:  # 計算済み部分の3時間前のファイルを読み込み
            date_5 = date_2 + datetime.timedelta(hours=-3)
            date_list.append(date_5)

            if date_2 <= start_date:
                break
        
            date_2 = date_5  # 前回読み込んだファイル（3～5時間）を0～2時間のファイルとする

    # 3時間ごとに計算, 最終から逆順
    date_2 = np.nan
    for date_5 in date_list:
        # ログ出力
        logging.info((date_5 + utc_jst).strftime(date_format) + '(JST)を計算しています')

        # GRIB2読込ファイル名の確定
        date_str = date_5.strftime(date_format)
        ...
        load_grib2_to_dataframe(grib2_dataset, date_5, df)

        # 絶対湿度の計算、日射量と大気放射量の推計を実施し、結果をpickleで保存
        date = hokankeisan(date_2, date_5, df, df_coef_atm, df_coef_rad, temp_path, logging)

        # 不要なデータをdfから削除
        df = clean_df(date_5, df, logging)
...
def hokankeisan(date_2, date_5, df, df_coef_atm, df_coef_rad, temp_path, logging):
    """
    ...
    date, _ = get_datetime_str(date_5, 2 + 9)  # JST補正（UTC+9）

    ###
    if date_2 is np.nan:  # 2ファイル分の読み込みが終わっていない1回目はパス
        pass
    else:
        for i in range(3):
            date = date_2 + datetime.timedelta(hours=i + 9)  # JST（UTC+9）に変更

            # 絶対湿度の計算、日射量と大気放射量の推計
            df_save = hokan_core(date, df, df_coef_atm, df_coef_rad)

            # 結果保存
            pickle_path = os.path.join(temp_path, date.strftime(date_format) + '.pkl')
...
def hokan_core(date, df, df_coef_atm, df_coef_rad):
    ...
    ### 重量絶対湿度を計算しDFに格納
    df.loc[:, date_str + '_MR'] = func_MR(df[date_str + '_TMP'].values,
                                          df[date_str + '_RH'].values,
                                          df[date_str + '_PRES'].values)

    ### 日射量・大気放射量の計算に使用する前時間との平均値を算出
    date_d1 = date + datetime.timedelta(hours=-1)
    date_d1_str = date_d1.strftime(date_format)

    TMP = (df[date_str + '_TMP'].values + df[date_d1_str + '_TMP'].values) / 2
    RH = (df[date_str + '_RH'].values + df[date_d1_str + '_RH'].values) / 2
    LCDC = (df[date_str + '_LCDC'].values + df[date_d1_str + '_LCDC'].values) / 200  # 雲量を100分率から0-1に換算
    MCDC = (df[date_str + '_MCDC'].values + df[date_d1_str + '_MCDC'].values) / 200
    HCDC = (df[date_str + '_HCDC'].values + df[date_d1_str + '_HCDC'].values) / 200
    TCDC = (df[date_str + '_TCDC'].values + df[date_d1_str + '_TCDC'].values) / 200
    LCDC5 = (df[date_str + '_LCDC5'].values + df[date_d1_str + '_LCDC5'].values) / 200
    LCDC25 = (df[date_str + '_LCDC25'].values + df[date_d1_str + '_LCDC25'].values) / 200
    HMCDC5 = (df[date_str + '_HMCDC5'].values + df[date_d1_str + '_HMCDC5'].values) / 200
    HMCDC25 = (df[date_str + '_HMCDC25'].values + df[date_d1_str + '_HMCDC25'].values) / 200

    df.loc[:, date_str + '_DSWRF_est'] = func_radiation(date,
                                                        df['lat'].values,
                                                        df['lon'].values,
                                                        HCDC,
                                                        MCDC,
    ...
    if date_str + '_DSWRF' in df.columns:
        df.loc[:, date_str + '_DSWRF_msm'] = df[date_str + '_DSWRF'].values * 3600 * 10 ** -6

    else:
        df.loc[:, date_str + '_DSWRF_msm'] = np.nan

    ### 大気放射量の推計
    df.loc[:, date_str + '_Ld'] = func_a_radiation(TMP,
                                                   RH,
                                                   LCDC5,
                                                   LCDC25,
                                                   HMCDC5,
                                                   HMCDC25,
                                                   df[date_str + '_APCP01'].values,
                                                   df_coef_atm)

    ### データを保存
    exp_list = ['index'] + [date_str + '_' + s for s in save_list]
    df_save = df[exp_list]
...
def load_grib2_to_dataframe(dataset, date, df):
    ...
    for fc_time in range(3):
        for weather in weather_set:
            ...
            data_temp, band_list = weather_selester(dataset, weather, fc_time, band_list)
            ...
            if data_temp is np.nan:
                ...
                df[date_str + '_' + weather] = in_series(data_temp).loc[index_list]

        ### 雲量を読み込んで、日射量・大気放射量計算用の雲量を計算してDFに格納
        _, date_str = get_datetime_str(date, fc_time + 9)    # JST補正（UTC+9）

        ### 低層雲量の読み込み
        LCDC_temp, band_list = weather_selester(dataset, 'LCDC', fc_time, band_list)
        # 平均処理なし
        df[date_str + '_LCDC5'] = in_series(LCDC_temp).loc[index_list]
        # 25㎞移動平均
        df[date_str + '_LCDC'] = in_series(ndimage.uniform_filter(LCDC_temp, size=5, mode='constant')).loc[
            index_list]
        # 形態係数の重みづけ平均
        df[date_str + '_LCDC25'] = in_series(signal.correlate(LCDC_temp, viewfactor(0.5), 'same')).loc[index_list]

        ### 中層雲量の読み込み
        MCDC_temp, band_list = weather_selester(dataset, 'MCDC', fc_time, band_list)
        df[date_str + '_MCDC'] = in_series(ndimage.uniform_filter(MCDC_temp, size=5, mode='constant')).loc[
            index_list]

        ### 高層雲量の読み込み
        HCDC_temp, band_list = weather_selester(dataset, 'HCDC', fc_time, band_list)
        df[date_str + '_HCDC'] = in_series(ndimage.uniform_filter(HCDC_temp, size=5, mode='constant')).loc[
            index_list]

        ### 全雲量の読み込み
        TCDC_temp, band_list = weather_selester(dataset, 'TCDC', fc_time, band_list)
        df[date_str + '_TCDC'] = in_series(ndimage.uniform_filter(TCDC_temp, size=5, mode='constant')).loc[
            index_list]
        ...
        # 形態係数の重みづけ平均
        df[date_str + '_HMCDC25'] = in_series(signal.correlate(HMCDC_temp, viewfactor(2.0), 'same')).loc[index_list]

def get_datetime_str(dt, timedifference):
    date = dt + datetime.timedelta(hours=timedifference)  # JST補正（UTC+9）
...
def init(args):
    ...
    # 計算期間
    start_date = datetime.datetime.strptime(args.start + '0000', date_format) + datetime.timedelta(hours=-9)
    end_date = pd.Timestamp(datetime.datetime.strptime(args.end + '0000', date_format)).ceil('3H') \
               + datetime.timedelta(hours=-9)

    # 計算対象のGRIB2ファイルをリストとして取得

    # GRIB2ファイルの一覧
    grib2_file_list = sorted(glob.glob(os.path.join(args.path_open, '*.bin')), reverse=True)
...
def main():
    ...
    parser.add_argument("--path_open", default='.{0}bin{0}'.format(os.path.sep))
    parser.add_argument("--path_save", default='.{0}msm{0}'.format(os.path.sep))
    parser.add_argument("--coef_rad_file",
                        help='重回帰係数(日射量・大気放射量の計算)',
                        default='.{0}data{0}coef_rad_20210125.csv'.format(os.path.sep))
    parser.add_argument("--coef_atm_file",
                        help='重回帰係数(大気放射量の推計)',
                        default='.{0}data{0}coef_arad_20210125.csv'.format(os.path.sep))
    parser.add_argument("--path_meshcode",
    ...
    # 補間計算ループ
    hokanloop(df, conf['date_end'], conf['df_coef_atm'], conf['df_coef_rad'], conf['grib2_file_list'], conf['temp_path'],
...
def split_files(MSM_list, temp_path, logger):
    ...
    for i in range(file_count):
        ...
        for name, group in df_group:
            save_path = os.path.join(temp_path, str(name), '{}_{}.pkl'.format(name, i))
